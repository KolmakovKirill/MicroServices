...
using microservices_project.Core.Domain.Services;
...
public class MediaController : ControllerBase
{
    ...
    public async Task<ActionResult<Media>> UploadMedia(IFormFile file)
    {
        try
        {
            var media = await _service.CreateMediaAsync(file);
    ...
    {
        var media = await _service.GetMediaAsync(id);
    ...
    {
        var success = await _service.DeleteMediaAsync(id);
